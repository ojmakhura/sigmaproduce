...
/**
 * 
 * TODO: Model Documentation for type.AccessPointTypeRestController
 */
public interface AccessPointTypeRestController {
    ...
    public ResponseEntity<?> pagedSearch(@PathVariable(name = "pageNumber", required = false) Integer pageNumber, @PathVariable(name = "pageSize", required = false) Integer pageSize, @RequestBody(required = true) String criteria);

    @DeleteMapping("/id/{id}")
    public ResponseEntity<?> remove(@PathVariable(name = "id", required = false) Long id);

    @PostMapping()
    public ResponseEntity<?> save(@RequestBody(required = true) AccessPointTypeVO accessPointType);

    @PostMapping("/search")
    public ResponseEntity<?> search(@RequestBody(required = false) String criteria);
}